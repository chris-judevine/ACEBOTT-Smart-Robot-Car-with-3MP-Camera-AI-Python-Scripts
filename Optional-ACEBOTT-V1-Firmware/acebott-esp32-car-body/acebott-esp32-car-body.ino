...
#define CMD_AVOID 6
#define CMD_FOLLOW 7
// added command auto stop
#define CMD_AUTO_STOP 14 // Hex 0x0E is 14
...
char model_var = 0;
int UT_distance = 0;
// add bool for auto-stop function
...
void loop() {
  ...
  if (autoStopActive) {
    middleDistance = Ultrasonic.Ranging();
    if (middleDistance <= 30 && middleDistance > 0) { // Safety check > 0
      Acebott.Move(Stop, 0);
      autoStopActive = false; // Deactivate until a new command is sent
    }
  }
  switch (newRunMode) {
    case 4:
